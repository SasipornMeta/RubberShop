...
public class PostBuyCube extends AsyncTask<String, Void, String>{
    private Context context;

    public PostBuyCube(Context context) {
    ...
    protected String doInBackground(String... strings) {
        try {
            ...
            RequestBody requestBody = new FormEncodingBuilder()
                    .add("isAdd", "true")
                    .add("b3_date", strings[0])
                    .add("c_id", strings[1])
                    .add("c_name", strings[2])
                    .add("b3_weight", strings[3])
                    .add("b3_percent", strings[4])
                    .add("b3_price", strings[5])
                    .add("b3_total", strings[6])
                    .build();
